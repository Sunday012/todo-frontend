...
type TodoData = {
    ...
    deadline: string;
    start_date: string;
    // setTodos: Dispatch<SetStateAction<undefined>>;
    todos: never[] | any[];
    title: string;
...
export const TodoCard = ({description, id, title, deadline, deadline_time} : TodoData) => {
  const {apiUrl} = useContext(AuthContext)
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // setTodos(arrayData.filter(todo => todo.rows.todo_id !== id))
            console.log(res)
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-[#333333] text-[#F2F2F2] rounded-lg">
      ...
      <div className="flex flex-col gap-4">
       <EditTodo description={description} id={id} />
        <Button className="bg-[#333333] hover:bg-[#808080] text-[#F2F2F2]" onClick={handleDelete}>
          <svg
            width="14"
            height="14"
            viewBox="0 0 13 14"
...
TodoCard.Skeleton = function SkeletonTodo(){
    return(
        <div className="w-6 h-6 p-4 rounded-lg">
